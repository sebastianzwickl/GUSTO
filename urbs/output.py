...
def get_constants(instance):
    ...
    for key in costs.keys():
        total_costs+=costs[key]

    co2_total = 0
    local_deficit = 0
    delta_load_following = 0
    m = instance
    stf = m.stf
    com = 'CO2'
    for t in m.tm:
        for s in m.sit:
            for p in m.pro:
                try:
                    co2_total += m.e_pro_out[t, stf, s, p, com].value * int(m.weight[t])
                except KeyError:
                    pass
                ...
                for tm in m.tm:
                    for sit in m.sit:
                        if com == 'Elec buy' and process == 'Supply from public grid':
                            local_deficit += m.e_pro_in[tm, stf, sit, process, com].value * int(m.weight[tm])
                        elif com == 'Elec sell' and process == 'Feed-in public grid':
                            local_deficit += m.e_pro_out[tm, stf, sit, process, com].value * int(m.weight[tm])

    #if m.mode['tra']:
    #    for tm in m.tm:
    #        for stf, sit1, sit2, tra, com in m.tra_tuples:
    #            local_deficit += m.e_tra_in[tm, stf, sit1, sit2, tra, com].value * m._data['weight']['Weight'][tm]


    for p, com in [['Wind|Onshore', 'Wind'], ['Solar|PV', 'Solar']]:
        for tm in m.tm:
            for s in m.sit:
                if m.obj.value == 'load_following' and (not math.isnan(m._data['declared'][s][com][tm])):
                    if m._data['declared'][s][com][tm] > ((m.cap_pro_new[stf, s, p].value +
                                                           m.process_dict['inst-cap']
                                                           [stf, s, p]) * m.supim_dict[s, com][stf, tm]):
                        delta_load_following += abs(m._data['declared'][s][com][tm] -
                                                    (m.cap_pro_new[stf, s, p].value + m.process_dict['inst-cap'][stf, s, p]) *
                                                    m.supim_dict[s, com][stf, tm] *
                                                    m.dt - m.e_sto_out[tm, stf, s, 'GSF '+p, 'Elec'].value) * \
                                                m._data['weight']['Weight'][tm]
                    elif m._data['declared'][s][com][tm] < ((m.cap_pro_new[stf, s, p].value +
                                                           m.process_dict['inst-cap']
                                                           [stf, s, p]) * m.supim_dict[s, com][stf, tm]):
                        delta_load_following += abs(m._data['declared'][s][com][tm] -
                                                    (m.cap_pro_new[stf, s, p].value + m.process_dict['inst-cap'][
                                                        stf, s, p]) *
                                                    m.supim_dict[s, com][stf, tm] *
                                                    m.dt + m.e_sto_in[tm, stf, s, 'GSF ' + p, 'Elec'].value) * \
                                                m._data['weight']['Weight'][tm]
                ...
                # else:
                    ...
                    # print(s)
                    # print(com)
                    # print(type(math.isnan(m._data['declared'][s][com][tm])))
                    # # print(math.isnan(m._data['declared'][s][com][tm]))  
                    # if (not math.isnan(m._data['declared'][s][com][tm])):
                        # delta_load_following += abs(m._data['declared'][s][com][tm] -
                                                # (m.cap_pro_new[stf, s, p].value + m.process_dict['inst-cap'][stf, s, p])
                                                # * m.supim_dict[s, com][stf, tm] * m.dt) * \
                                            # m._data['weight']['Weight'][tm]
    data_co2 = {
        'Global values' : [total_costs, co2_total, local_deficit, delta_load_following],
        'Variable': ['Total costs', 'Total emission', 'Local deficit/excess', 'Scheduled deviation'],
        'Unit': ['EUR/a', 'tCO2/a', 'kWh', 'kWh']
    }
    ...
    return costs, cpro, ctra, csto, df
