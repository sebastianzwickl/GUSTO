...
def create_model(data, dt=1, timesteps=None, objective='costs',
                 dual=True):
    if not timesteps:
        timesteps = data['demand'].index.tolist()  # length of the time-series data

    m = pyomo_model_prep(data, timesteps)  # preparing pyomo mode
    m.name = 'GUSTO'
    m.created = datetime.now().strftime('%Y%m%dT%H%M')
    m._data = data # read in data from input/... (excel file)
    ...
    m.cap_pro_new_binary = pyomo.Var(
            m.pro_tuples,
            within=pyomo.Binary,
            doc='binary decision variable whether technology is installed or not (true(1)/false(0))')

    m.cap_new_binary_expression = pyomo.Constraint(
        m.pro_tuples,
        rule=def_binary_process,
        doc='newly installed capacity <= binary decision variable * upper capacity bound'
    ...
    if m.obj.value == 'cost':
        print('Objective function: minimize total costs of supply (1)')
        if m.mode['int']:
        ...
        print('Objective function: minimize total CO2 emissions (x)')
    ...
    elif m.obj.value == 'local':
        print('Objective function: minimize (total) local excess/deficit (2)')
        # Add constraint that limits maximum of total costs
        m.res_global_cost_limit = pyomo.Constraint(
            rule=res_global_cost_limit_rule,
            doc='total costs <= Global cost limit')
        # Set the objective function (Local Self-consumption) accordingly
        m.objective_function = pyomo.Objective(
            rule=minimize_grid_supply,
            sense=pyomo.minimize,
            doc='minimize total supply from the public grid')

    elif m.obj.value == 'load_following':
        print('Objective function: minimize total costs of supply (3) with load following restrictions')
        m.objective_function = pyomo.Objective(
            rule=cost_rule,
            sense=pyomo.minimize,
            doc='minimize(cost = sum of all cost types)')
        m.exr_load_following = pyomo.Constraint(
            m.tm, m.pro_input_tuples,
            rule=load_following_constraint,
            doc='actual feed-in + local storage system <= declared feed-in * (1 + Permitted Deviation)'
                '(from renewable resources)')
        m.exr_load_following_lower = pyomo.Constraint(
            m.tm, m.pro_input_tuples,
            rule=load_following_constraint_lower,
            doc='actual feed-in + local storage system >= declared feed-in * (1 - Permitted Deviation)'
                '(from renewable resources)')
    else:
        raise NotImplementedError("Non-implemented objective quantity. Set "
                                  "either 'cost', 'CO2', or 'local' as the objective in "
                                  "runme.py!")
    if dual:
...
def res_stock_total_rule(m, stf, sit, com, com_type):
    if com not in m.com_stock:
        ...
        for tm in m.tm:
            total_consumption += (
                m.e_co_stock[tm, stf, sit, com, com_type]*int(m.weight[tm]))
...
def res_env_total_rule(m, stf, sit, com, com_type):
    if com not in m.com_env:
        ...
        env_output_sum = 0
        for tm in m.tm:
            env_output_sum += (- commodity_balance(m, tm, stf, sit, com)*int(m.weight[tm]))
...
# newly installed capacity <= binary decision variable (1/0) * upper bound of capacity per process technology
def def_binary_process(m, stf, sit, pro):
    return m.cap_pro_new[stf, sit, pro] <= m.cap_pro_new_binary[stf, sit, pro]*m.process_dict['cap-up'][(stf, sit, pro)]
...
def res_global_cost_limit_rule(m):
    # Check whether the value of maximal total costs is set or not
...
def def_costs_rule(m, cost_type):
    ...
    if cost_type == 'Invest':
        cost = \
            sum((m.cap_pro_new[p] *
                m.process_dict['inv-cost'][p]+m.cap_pro_new_binary[p] * m.process_dict['con-cost'][p]) * m.process_dict['invcost-factor'][p]
        ...
        cost = \
            sum(m.tau_pro[(tm,) + p] * int(m.weight[tm]) *
        ...
        return m.costs[cost_type] == sum(
            m.e_co_stock[(tm,) + c] * int(m.weight[tm]) *
        ...
        return m.costs[cost_type] == sum(
            - commodity_balance(m, tm, stf, sit, com) * int(m.weight[tm]) *
        ...
        for tm in m.tm:
            for sit in m.sit:
                ...
                co2_output_sum += - commodity_balance(m, tm, stf, sit, 'CO2') * \
                                  m._data['weight']['Weight'][tm]
    return co2_output_sum


# (2)nd operation strategy: minimization of local excess/deficit by prosumers
def minimize_grid_supply(m):
    total_grid_exchange = 0
    for process in ['Supply from public grid', 'Feed-in public grid']:
        for com in ['Elec buy', 'Elec sell']:
            for stf in m.stf:
                for tm in m.tm:
                    for sit in m.sit:
                        if com == 'Elec buy' and process == 'Supply from public grid':
                            total_grid_exchange += m.e_pro_in[tm, stf, sit, process, com] * \
                                                   m._data['weight']['Weight'][tm]
                        elif com == 'Elec sell' and process == 'Feed-in public grid':
                            total_grid_exchange += m.e_pro_out[tm, stf, sit, process, com] * \
                                                   m._data['weight']['Weight'][tm]

    # Change objective function to the neighborhood perspective
    #if m.mode['tra']:
    #    for tm in m.tm:
    #        for stf, sit1, sit2, tra, com in m.tra_tuples:
    #            total_grid_exchange += m.e_tra_in[tm, stf, sit1, sit2, tra, com] * m._data['weight']['Weight'][tm]
    return total_grid_exchange

# (3)rd operation strategy: load following by using battery storage capacities - this includes dumb or smart EV charging
# upper bound for load following
def load_following_constraint(m, tm, stf, sit, pro, coin):
    if coin in m.com_supim:
        if not math.isnan(m._data['declared'][sit][coin][tm]):
            try:
                return (
                    m.cap_pro[stf, sit, pro] * m.supim_dict[sit, coin][stf, tm] * m.dt +
                    m.e_sto_out[tm, stf, sit, 'GSF '+pro, 'Elec']  - m.e_sto_in[tm, stf, sit, 'GSF '+pro, 'Elec']
                    <= m._data['declared'][sit][coin][tm] * \
                       (1 + m._data['global_prop']['value'][stf, 'Load following']/100))
            except:
                raise NotImplementedError("Not enough storage technologies implemented.")
        else:
            return pyomo.Constraint.Skip
    else:
        return pyomo.Constraint.Skip

# lower bound for load following
def load_following_constraint_lower(m, tm, stf, sit, pro, coin):
    if coin in m.com_supim:
        if not math.isnan(m._data['declared'][sit][coin][tm]):
            try:
                return (
