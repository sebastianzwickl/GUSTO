...
def read_input(input_files, year):
    ...
    for filename in input_files:
        with pd.ExcelFile(filename) as xls:
            ...
            if 'Weight' in xls.sheet_names:
                weight_factor = xls.parse('Weight').set_index(['t'])
                weight_factor = pd.concat([weight_factor], keys=[support_timeframe],
                                          names=['support_timeframe'])
                w.append(weight_factor)

            if 'Declared generation' in xls.sheet_names:
                declared = xls.parse('Declared generation').set_index(['t'])
                declared = pd.concat([declared], keys=[support_timeframe], names=['support_timeframe'])
                declared.columns = split_columns(declared.columns, '.')
                dec.append(declared)
    ...
    try:
        ...
        weight = pd.concat(w, sort=False)
        declared = pd.concat(dec, sort=False)
