...
def validate_input(data):
    ...
    # identify representation of annual time-series
    if not data['weight'].empty:
        if not (52 <= np.sum(data['weight']['Weight'])/168 <= 53):
            raise KeyError('Time step weight must represent annual time series: '
                           'sum of time-step weights must be either 52 or 53 (leap year) which is the number '
                           'of weeks of a year. Current value:',np.sum(data['weight']['Weight'])/168)
